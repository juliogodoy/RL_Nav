...
class RVOEnv():
    ...
    def __init__(self):
        ...
        self.a0 = sim.addAgent((5, 8))
        self.a1 = sim.addAgent((9, 8), 15, 10, 5, 1.3, 0.4, 1.5, (0, 0))
        ...
        self.o1 = sim.addObstacle([(18.0, 100.0), (4.0, 100.0), (4.0, 8.42),(18.0, 8.42)])
        self.o2 = sim.addObstacle([(18.0, 7.58), (4.0, 7.58), (4.0, -100.0),(18.0, -100.0)])
    ...
    def render(self):
        ...
        obstacle_pos=np.array([4,8.42])
        obstacle_size=np.array([18,100])
        draw_AABB(self.screen, *meters2pixels(obstacle_pos),*meters2pixels(obstacle_size), BLUE)
        obstacle_pos=np.array([4,-100])
        obstacle_size=np.array([18,7.58])
    ...
    def step(self, action):
        ...
        for agent_no in range(2):
            ...
            #sim.setAgentPrefVelocity(agent_no, ((1-agent_no*0.3)*(unitario[0]+dist*math.cos(angle)), unitario[1]+dist*math.sin(angle)))
        #print("Action: ", action)
        ...
        obstacle0_relative_p= np.array([18.0, 100.0])-np.array(agent_own_p)
        obstacle1_relative_p= np.array([4.0, 100.0])-np.array(agent_own_p)
        obstacle2_relative_p= np.array([4.0, 8.42])-np.array(agent_own_p)
        obstacle3_relative_p= np.array([18.0, 8.42])-np.array(agent_own_p)
        obstacle4_relative_p= np.array([18.0, 7.58])-np.array(agent_own_p)
        obstacle5_relative_p= np.array([4.0, 7.58])-np.array(agent_own_p)
        obstacle6_relative_p= np.array([4.0, -100.0])-np.array(agent_own_p)
        obstacle7_relative_p= np.array([18.0, -100.0])-np.array(agent_own_p)
        goal_relative_p=np.array([11,8])- np.array(agent_own_p)
        ...
        done = (np.linalg.norm(np.array([11,8])- np.array(sim.getAgentPosition(0))) < 1)# and (np.linalg.norm(np.array([-10,8])- np.array(sim.getAgentPosition(1))) < 1)
        #print("Done is ", done)
    ...
    def reset(self):
        ...
        sim.setAgentPosition(1, (9-initialPosProbX*2,8))

        collision=True
        while collision:
            initialPosProbX= random.random()
            initialPosProbY= random.random()
            initiapPosRLAgent= tuple((11-initialPosProbX*7,8))
            #print(" Trying position: ", initiapPosRLAgent)
            collision = isCircleCircleColliding(initiapPosRLAgent[0],initiapPosRLAgent[1],0.4, sim.getAgentPosition(1)[0],sim.getAgentPosition(1)[1],0.4)
        #if initiapPosRLAgent[0]>9:
            #print("Starting: ", initiapPosRLAgent)

        sim.setAgentPosition(0,initiapPosRLAgent)
        #sim.setAgentPosition(0, (5.5-initialPosProbX,6+initialPosProbY*4))
